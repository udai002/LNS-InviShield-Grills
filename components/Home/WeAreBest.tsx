...
export const FeatureCard = ({ icon, title, description, variant = "dark", index }: FeatureCardProps) => {
  ...
  return (
    <motion.div
      ...
      className={`
        relative rounded-2xl p-6 flex flex-col gap-4 border overflow-hidden cursor-default ${bgClass}
      `}
      style={{ boxShadow: "0 8px 28px rgba(0,0,0,0.14)" }}
    >
      {/* (colored background via gradient) */}
      ...
      <motion.div
        ...
        animate={{
          ...
          background: spotlight.show
            ? `radial-gradient(180px circle at ${spotlight.x}px ${spotlight.y}px, rgba(255,255,255,0.06), transparent 80%)`
            : "none",
        }}
      ...
      <motion.div
        ...
        className={`
          relative z-10 w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 backdrop-blur-sm bg-white/20 text-white
        `}
      >
      ...
      {/* Animated corner dot */}
      <motion.div
        className={`absolute top-4 right-4 w-1.5 h-1.5 rounded-full z-10 bg-white/90`}
